...
document.querySelectorAll('[data-card]').forEach((card) => {
  card.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.ff-rail__nav')) return;
    if (card.hasAttribute('aria-hidden')) return;
    const wasOpen = card.classList.contains('ff-is-open');
    card.classList.toggle('ff-is-open');

    if (!wasOpen && window.amplitude) {
      const title = card.querySelector('.ff-card__title');
...
document.querySelectorAll('[data-rail]').forEach((rail) => {
  const viewport = rail.querySelector('.ff-rail__viewport');
  const track = rail.querySelector('.ff-rail__track');
  const prevBtn = rail.querySelector('.ff-rail__nav--prev');
  const nextBtn = rail.querySelector('.ff-rail__nav--next');
  ...
  const stepSize = () => {
    const card = track.querySelector('.ff-card--rail');
...
document.querySelectorAll('.ff-partner, .ff-stat, .ff-aud, .ff-vendor').forEach((el) => {
  el.style.opacity = '0';
...
(function () {
  ...
  // OTT button sits under an invisible Sweatpals iframe inside .ff-rsvp-trigger,
  ...
  // Hero secondary CTA
  const heroSee = document.querySelector('.ff-hero__ctas a[href="#programming"]');
  if (heroSee) {
